...
def subsample(X, y, max_n: int, rng: np.random.Generator, extra=None):
    ...
    NOTE: this is a FLAT total cap — sklearn's stratify=y allocates the max_n budget
    proportionally to each class's frequency in the input, not equally. A rare class
    can end up with very few (even 0-ish) samples if it's a small fraction of the
    input. For a true per-class quota, use subsample_per_class instead — that's what
    max_train_per_class/max_test_per_class configs use (see experiment_common.py
    ::_load_and_cap); this flat version backs max_train_samples/max_test_samples
    (river's flat-total convention) only.
...
def subsample_per_class(X, y, max_per_class: int, rng: np.random.Generator, extra=None):
    """True per-class cap: up to max_per_class samples for EACH class independently —
    not a flat total allocated proportionally (see subsample's note above). A class
    with fewer than max_per_class available samples keeps all of them (no error, no
    padding, no borrowing from other classes) — the resulting dataset is only
    "balanced" up to whatever each class actually has.
    ...
    for c in np.unique(y):
        ...
        all_idx.append(cls_idx)
    idx = np.sort(np.concatenate(all_idx)) if all_idx else np.array([], dtype=int)
    sub_extra = extra[idx] if extra is not None else None
    if extra is not None:
        return [X[i] for i in idx], y[idx], sub_extra
