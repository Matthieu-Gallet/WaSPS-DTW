...
7-value grid used elsewhere), >=10 seeds per test. Reuses optimize_gamma.py's
grid_knn unmodified — a pure gamma search per test config, same machinery the
baseline experiment's gamma calibration uses.
...
Usage:
    python src/dev/analysis/analyze_wasps_vs_euclraw.py --n-jobs 2
"""
...
_EXPERIMENT = _HERE.parent.parent / "experiment"        # src/dev/analysis -> src/dev -> src -> experiment
sys.path.insert(0, str(_EXPERIMENT / "utils"))
sys.path.insert(0, str(_EXPERIMENT))

from experiment_common import _log
from optimize_gamma import grid_knn
