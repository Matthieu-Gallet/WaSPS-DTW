"""Per-dataset .npy dump for Experiment 1 / 1bis: raw train/test time series + labels
(both experiments), plus fitted per-class barycenters (barycenter experiment only).

Raw series are method-independent for a given (dataset, seed) — _load_and_cap's RNG depends
only on seed, not method — so this reads the data once via _load_and_cap directly, entirely
separate from experiments/run_full_baseline.py's own narrow --bary-plot-dataset/--bary-plot-
methods mechanism (which keeps feeding plot_exp1_river_bary.py unchanged).

Usage:
    python experiments/dump_arrays.py --config configs/exp1_baseline.yaml --dataset river \\
        --seed 42 --out-series results/jax_exp1_knn_baseline/river_series.npy

    python experiments/dump_arrays.py --config configs/exp1_baseline.yaml --dataset cpazmal \\
        --seed 42 --out-series results/jax_exp1bis_bary_baseline/cpazmal_series.npy \\
        --out-barycenters results/jax_exp1bis_bary_baseline/cpazmal_barycenters.npy \\
