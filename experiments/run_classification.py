"""Classification experiment runner: multi-method × 2-mode (KNN + Barycenter) × multi-seed × k-fold.
...
Usage:
    ...
    python experiments/run_classification.py configs/river.yaml --n-jobs -1 --verbose --debug

Outputs to <output.dir>/:
  classification_scores.csv        — mean ± std over (fold,seed) [+ gamma column if swept]
  classification_scores_detail.csv — one row per (method,mode,k[,gamma])×(fold,seed):
                                      individual F1, per-class F1 (f1_class_<c>), timings
...
Methods (defined once, experiments/method_defs.py):

  wasps              — WaSPS W₂² on estimated params  (use_positivity_constraint, manual grad)
  wasps_nodiv        — same, is_divergence=False + log_correction=False
  eucl_params        — SqEuclidean on estimated params (autodiff)
  eucl_params_nodiv  — same, is_divergence=False
...
Repetition: `n_seeds` (independent train/test draws) × k-fold (cross_validation.n_splits
with fold=-1 looping all folds, optionally n_seeds_per_fold nested repeats) — same
(seed,fold) iteration convention as experiments/run_sensitivity.py. Reported metrics =
NaN-safe mean ± std across all (fold,seed) iterations.

`classification.gamma_values` (optional list): when present, sweeps gamma as an outer
axis in addition to k — every output row gets a `gamma` column. Absent → single-gamma
behaviour identical to before this option existed.

`classification.estimator` (optional, default 'mle'): passed through to build_repr for
'params'-repr methods ('mle' or 'log_cumulant' — see CLAUDE.md for why log_cumulant is
markedly more stable for WaSPS barycenter fitting on wide-range real data).

--n-jobs (default 1, sequential): when != 1, iterations (seed,fold) run as parallel
joblib jobs (backend='loky'), matching run_sensitivity.py's parallelization granularity.
--debug/--verbose: same EXPERIMENT_LOG_FILE/EXPERIMENT_DEBUG/EXPERIMENT_VERBOSE env-var
logging mechanism as run_sensitivity.py/run_optim_hyper.py (see experiment_common.py).
This is independent from the pre-existing `cfg["debug"]` config key, which triggers the
heavier one-shot dataset/barycenter diagnostic dump (_debug_dump_dataset/_debug_dump_barycenters).
"""
...
from experiment_common import _log, _eval_metrics, _write_csv, _write_detail_csv
...
def _run_one_seed(cfg: dict, seed: int, methods: list, fold: int | None = None) -> list:
    ...
    results     = []
    # Cache: (repr_type, split) → (repr_list, filtered_labels). Shared across gamma
    # values (repr build doesn't depend on gamma) and across methods with the same
    # repr_type (e.g. wasps/eucl_params both 'params') — so train_time/test_time
    # below reflect the true marginal cost only for the first method/gamma that
    # triggers the build; later cache hits report ~0, which is accurate (no work done).
    _repr_cache: dict = {}
    ...
    def _get_repr(repr_type, split):
        if (repr_type, split) not in _repr_cache:
            ...
            _repr_cache[(repr_type, split)] = build_repr(raw, lbl, repr_type, family, estimator)
    ...
    for method in methods:
        ...
        t_train_repr = time.time() - t_r0
        t_r1 = time.time()
        test_repr,  test_repr_labels  = _get_repr(repr_type, 'test')
        t_test_repr = time.time() - t_r1
        ...
        if 'knn' in modes:
            for gamma in gamma_values:
                for k in k_values:
                    ...
                    t_knn = time.time() - t0
                    train_time = t_train_repr
                    test_time  = t_test_repr + t_knn
                    res = _metrics(preds, truth)
                    res["f1_per_class"] = _eval_metrics(preds, truth)["f1_per_class"]
                    res.update({"method": method, "mode": "knn", "k": k, "gamma": gamma,
                                "seed": seed, "fold": fold, "time_s": train_time + test_time,
                                "train_time": train_time, "test_time": test_time})
                    results.append(res)
                    print(f"  [{method}/knn k={k} gamma={gamma:.4g}] acc={res['accuracy']:.3f}  "
                          f"f1={res['f1_weighted']:.3f}  t={train_time + test_time:.1f}s", flush=True)
        ...
        if 'barycenter' in modes:
            for gamma in gamma_values:
                ...
                train_time = t_train_repr + (time.time() - t0)

                if _debug and _is_first and gamma == gamma_values[0]:
                ...
                test_time = t_test_repr + (time.time() - t1)
                res = _metrics(preds, test_repr_labels)
                res["f1_per_class"] = _eval_metrics(preds, test_repr_labels)["f1_per_class"]
                res.update({"method": method, "mode": "barycenter", "k": None, "gamma": gamma,
                            "seed": seed, "fold": fold,
                            "time_s": train_time + test_time,
                            "train_time": train_time, "test_time": test_time,
                            "train_time_s": train_time, "infer_time_s": test_time})
                results.append(res)
                print(f"  [{method}/bary gamma={gamma:.4g}] acc={res['accuracy']:.3f}  "
...
def _nanmeanstd(vals: list) -> tuple:
    arr = np.array(vals, dtype=float)
    ok = ~np.isnan(arr)
    if not ok.any():
        return float('nan'), float('nan'), 0
    return float(np.nanmean(arr)), float(np.nanstd(arr)), int(ok.sum())
...
def _aggregate(all_results: list) -> list:
    ...
    for r in all_results:
        ...
        # gamma=None bucket key when gamma wasn't swept (single value) keeps the
        # summary CSV schema unchanged for existing configs (no gamma_values set).
        buckets[(r["method"], r["mode"], r.get("k"), r.get("gamma"))].append(r)
    summary = []
    for (method, mode, k, gamma), rows in sorted(
            buckets.items(), key=lambda x: (x[0][0], x[0][1], x[0][2] or -1, x[0][3] or -1)):
        acc_mean,  acc_std,  n_ok = _nanmeanstd([r["accuracy"]    for r in rows])
        f1_mean,   f1_std,   _    = _nanmeanstd([r["f1_weighted"] for r in rows])
        time_mean, time_std, _    = _nanmeanstd([r["time_s"]      for r in rows])
        train_time_mean, _, _ = _nanmeanstd([r.get("train_time", float('nan')) for r in rows])
        test_time_mean,  _, _ = _nanmeanstd([r.get("test_time", float('nan'))  for r in rows])
        entry = {
            "method": method, "mode": mode, "k": k, "gamma": gamma, "n_seeds": len(rows), "n_ok": n_ok,
            "acc_mean": acc_mean, "acc_std": acc_std,
            "f1_mean":  f1_mean,  "f1_std":  f1_std,
            "time_mean": time_mean, "time_std": time_std,
            "train_time_mean": train_time_mean, "test_time_mean": test_time_mean,
        }
...
def main(config_path: str, n_jobs: int = 1, verbose: bool = False, debug: bool = False,
    ...
    os.environ["EXPERIMENT_LOG_FILE"] = str(out_dir / "classification.log")
    if debug:
    ...
    if n_splits > 1 and fold_cfg < 0:
        ...
        _log(f"[run] K-fold mode: n_splits={n_splits}  group_aware={cv.get('group_aware', False)}  "
             f"n_seeds_per_fold={n_seeds_per_fold}")
    else:
        fold_arg = fold_cfg if n_splits > 1 else None
        iterations = [(seed, fold_arg) for seed in seeds]

    gamma_values = cfg.get("classification", {}).get("gamma_values")
    _log(f"[run] dataset={cfg['dataset']['type']}  family={family}  methods={methods}  "
         f"n_iter={len(iterations)}  n_jobs={n_jobs}"
         + (f"  gamma_values={gamma_values}" if gamma_values else ""))
    ...
    if n_jobs == 1:
        ...
        results_list = Parallel(n_jobs=n_jobs, backend='loky', verbose=10)(
            delayed(_run_one_seed)(cfg, seed, methods, fold=fold) for seed, fold in iterations
        )
    ...
    fields = ["method", "mode", "k", "gamma", "n_seeds", "n_ok",
              "acc_mean", "acc_std", "f1_mean", "f1_std",
              "time_mean", "time_std", "train_time_mean", "test_time_mean"]
    _write_csv(csv_path, fields, summary)

    detail_rows = []
    for r in all_results:
        row = {"method": r["method"], "mode": r["mode"], "k": r.get("k"), "gamma": r.get("gamma"),
              "seed": r["seed"], "fold": r["fold"], "f1": r["f1_weighted"],
              "train_time": r.get("train_time"), "test_time": r.get("test_time"),
              "total_time": r.get("time_s")}
        row.update({f"f1_class_{c}": v for c, v in r.get("f1_per_class", {}).items()})
        detail_rows.append(row)
    _write_detail_csv(out_dir / "classification_scores_detail.csv",
                      ["method", "mode", "k", "gamma", "seed", "fold",
                       "f1", "train_time", "test_time", "total_time"], detail_rows)
    ...
    for row in summary:
        ...
        g_tag = f" gamma={row['gamma']:.4g}" if row.get("gamma") is not None else ""
        _log(f"  {row['method']:12s}/{row['mode']:10s}{k_tag}{g_tag}  "
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="WaSPS-DTW classification experiment runner")
    parser.add_argument("config", help="e.g. configs/classification/river.yaml")
    parser.add_argument("--n-jobs", type=int, default=5,
                        help="joblib Parallel over (seed,fold), capped at 5 by default "
                             "(pass 1 for sequential); barycenter mode's class-level fit is a "
                             "separate, always-on inner parallelism capped at 4 (experiment_common._eval_bary)")
    parser.add_argument("--seed", type=int, default=None, help="override config's base seed")
    parser.add_argument("--debug", action="store_true",
                        help="log per-(fold,seed) resolved params to the log file "
                             "(independent of the pre-existing cfg['debug'] dataset/bary dump)")
    parser.add_argument("--verbose", action="store_true",
                        help="save barycenter arrays (.npz) + plots (.png) to "
                             "out_dir/bary_debug/ (barycenter mode only)")
